...
float converter(float v) {
  ...
  return channel == ivec2(1, 0)
    ? step(time, v) // 中央下：二階調化
    : channel == ivec2(2, 0)
      ? (floor(n * v) + step(0.5, fract(n * v))) / n // 右下：ポスタリゼーション
      : channel == ivec2(0, 1)
        ? smoothstep(0.5 * (1.0 - time), 0.5 * (1.0 + time), v) // 左上：S字トーンカーブ
        : channel == ivec2(1, 1)
          ? pow(v, 2.0 * time) // 中央上：ガンマ補正
          : channel == ivec2(2, 1)
            ? 0.5 * sin(4.0 * PI * v + u_time) + 0.5 // 右上：ソラリゼーション
            : v; // 左下：元画像
}
