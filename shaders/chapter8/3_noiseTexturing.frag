...
float text(vec2 st) {
  float time = 0.3 * u_time; // 時間変数
  float v0 = warp21(st + time, 1.0); // fBMのドメインワーピング
  float v1 = fdist31(vec3(st + time, time)); // 第1近傍距離を使った胞体ノイズ
  time = abs(mod(time, 2.0) - 1.0); // [0,1]区間を連続に動くように変換
  return mix(v0, v1, smoothstep(0.25, 0.75, time)); // 2つのノイズのモーフィング
}
void main() {
  vec2 p =
